...
int main(int argc, char **argv)
{
    ...
    uim_send_apdu_info_t input_apdu = {0};
    uim_send_apdu_response_t out_apdu;
    uint16_t sdk_error;
    char *aid = NULL;
    ...
    if (out.channel_id_available) {
        ...
        sleep(1);
        input_apdu.slot = 1;
        input_apdu.channel_id_available = true;
        input_apdu.channel_id = out.channel_id;
        input_apdu.procedure_bytes_setting_available = true;
        input_apdu.procedure_bytes_setting = QMI_UIM_APDU_SKIP_PROCEDURE_BYTES;
        input_apdu.apdu_sz = ascii_to_hex("83E2910012BF2D0F5C0D5A4F9F709091929395B6B7B89900",
                strlen("83E2910012BF2D0F5C0D5A4F9F709091929395B6B7B89900"), input_apdu.apdu);
        ret = tqcm_uim_send_apdu_send(&ctx_uim,
                &input_apdu,
                &out_apdu,
                &sdk_error,
                NULL,
                0);
        if (ret != 0) {
            RLOGE("%s: tqcm_uim_send_apdu_send fail %d", __FUNCTION__, ret);
            if (ret == EPROTO) {
            ...
            if (out_apdu.apdu_sz) {
                telit_print_buffer(out_apdu.apdu, out_apdu.apdu_sz, LOG_DEBUG);
            } else
                RLOGI("Empty response");
        }

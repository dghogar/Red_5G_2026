...
int main(int argc, char **argv)
{
    ...
    ret = tqcm_dms_get_device_serial_numbers_send(&ctx_dms,
            &out,
            &sdk_error,
            NULL,
            0);
    ...
    if (out.esn_len) {
        RLOGI("DMS esn %s", out.esn);
    }
    if (out.imei_len) {
        RLOGI("DMS imei %s", out.imei);
    }
    if (out.meid_len) {
        RLOGI("DMS meid %s", out.meid);
    }
    if (out.imei_svn_len) {
        RLOGI("DMS imei_svn %s", out.imei_svn);
    }

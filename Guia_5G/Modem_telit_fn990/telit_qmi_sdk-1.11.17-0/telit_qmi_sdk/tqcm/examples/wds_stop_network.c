...
int main(int argc, char **argv)
{
    ...
    wds_get_packet_service_status_t pkt_svc_status = { 0 };
    wds_stop_network_info_t stop_info = { 0 };
    uint8_t i;
    uint8_t index;
    ...
    RLOGI("Stopping network for profile %u", index);
    ...
    if (out_get.profile_settings.ipv6_addr_available) {
        ...
        for (i = 0; i < IPV6_ADDR_SIZE; i++) {
            RLOGI("%u ", ntohs(out_get.profile_settings.ipv6_addr.addr[i]));
        }
        RLOGI("Address prefix length: %u", out_get.profile_settings.ipv6_addr.prefix_len);
    }

    if (out_get.profile_settings.ipv6_gw_addr_available) {
        RLOGI("ipv6 gw address: ");
        for (i = 0; i < IPV6_ADDR_SIZE; i++) {
            RLOGI("%u ", ntohs(out_get.profile_settings.ipv6_gw_addr.addr[i]));
        }
        RLOGI("Address prefix length: %u", out_get.profile_settings.ipv6_gw_addr.prefix_len);
    }

    if (out_get.profile_settings.pri_dns_ipv6_addr_available) {
        RLOGI("ipv6 primary dns address: ");
        for (i = 0; i < IPV6_ADDR_SIZE; i++) {
            RLOGI("%u ", ntohs(out_get.profile_settings.pri_dns_ipv6_addr[i]));
        }
    }
    ...
    if (out_get.profile_settings.sec_dns_ipv6_addr_available) {
        ...
        for (i = 0; i < IPV6_ADDR_SIZE; i++) {
            RLOGI("%u ", ntohs(out_get.profile_settings.sec_dns_ipv6_addr[i]));
        }
    }
    ...
    ret = tqcm_wds_get_pkt_srvc_status_send(&ctx_wds, &pkt_svc_status, &sdk_error, NULL, 0);
    ...
    RLOGI("Stopping network (handle: %u)", stop_info.pkt_data_handle);

    ret = tqcm_wds_stop_network_interface_send(&ctx_wds, &stop_info, &sdk_error, NULL, 0);

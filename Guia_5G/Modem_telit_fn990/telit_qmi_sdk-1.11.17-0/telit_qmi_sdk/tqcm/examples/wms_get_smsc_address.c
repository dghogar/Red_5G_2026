...
int main(int argc, char **argv)
{
    ...
    ret = tqcm_wms_get_smsc_address_send(&ctx_wms, &out, &sdk_error, NULL, 0);
    ...
    if (out.smsc_address_type_len > 0) {
        RLOGI("smsc_address_type: %s", out.smsc_address_type);
    }
    if (out.smsc_address_len > 0) {
        RLOGI("smsc_address: %s", out.smsc_address);
    }

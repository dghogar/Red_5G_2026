...
static void print_call_info(voice_get_all_call_info_t *out)
{
    int i;

    if (!out->call_info_size) {
        RLOGI("no call info found");
    }
    ...
    if (!out->remote_party_number_size) {
        RLOGI("no array remote party number found");
    }
    for (i = 0; i < out->remote_party_number_size; ++i) {
        RLOGI("array remote party number [%d]", i);
        RLOGI("  call_id    %u", out->remote_party_number[i].call_id);
        RLOGI("  number pi  %u", out->remote_party_number[i].number_pi);
        RLOGI("  number     %s (%d)", out->remote_party_number[i].number,
                out->remote_party_number[i].number_len);
    }

    if (!out->remote_party_name_size) {
        RLOGI("no array remote party name found");
    }
    for (i = 0; i < out->remote_party_name_size; ++i) {
        RLOGI("array remote party name [%d]", i);
        RLOGI("  call_id  %u", out->remote_party_name[i].call_id);
        RLOGI("  name pi  %u", out->remote_party_name[i].name_pi);
        RLOGI("  coding   %u", out->remote_party_name[i].coding);
        RLOGI("  name     %s (%d)", out->remote_party_name[i].name,
                out->remote_party_name[i].name_len);
    }

    if (!out->array_uus_size) {
        RLOGI("no array UUS info found");
    }
    for (i = 0; i < out->array_uus_size; ++i) {
        int j;
        RLOGI("array UUS info [%d]", i);
        RLOGI("  call_id  %u", out->array_uus[i].call_id);
        RLOGI("  name pi  %u", out->array_uus[i].uus_type);
        RLOGI("  coding   %u", out->array_uus[i].uus_dcs);
        RLOGI("  data len %u", out->array_uus[i].uus_data_len);
        for (j = 0; j < out->array_uus[i].uus_data_len; ++j) {
            RLOGI("0x%04X,", out->array_uus[i].uus_data[j]);
        }
    }

    if (!out->conn_party_number_size) {
        RLOGI("no conn party number info found");
    }
    for (i = 0; i < out->conn_party_number_size; ++i) {
        RLOGI("conn party number info [%d]", i);
        RLOGI("  call_id  %u", out->conn_party_number[i].call_id);
        RLOGI("  name pi  %u", out->conn_party_number[i].conn_num_pi);
        RLOGI("  name si  %u", out->conn_party_number[i].conn_num_si);
        RLOGI("  type     %u", out->conn_party_number[i].conn_num_type);
        RLOGI("  name     %s (%d)", out->conn_party_number[i].conn_num,
                out->conn_party_number[i].conn_num_len);
    }
}
...
int main(int argc, char **argv)
{
    ...
    tqcm_client_context_t ctx_voice = {0};
    voice_indications_t ind = {0};
    voice_get_all_call_info_t out = {0};
    uint16_t sdk_error;
    ...
    ret = tqcm_voice_get_all_call_info_send(&ctx_voice,
            &out,
            &sdk_error,
            NULL,
            0);

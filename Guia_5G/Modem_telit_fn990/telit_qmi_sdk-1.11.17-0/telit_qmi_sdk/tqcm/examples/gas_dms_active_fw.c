...
int main(int argc, char **argv)
{
    ...
    uint16_t sdk_error;
    gas_dms_active_fw_t input = {0};

    input.carrier_name_available = true;
    strlcpy(input.carrier_name.name, "Generic", sizeof(input.carrier_name.name));
    input.carrier_name.name_len = strlen("Generic");
    ...
    ret = tqcm_gas_dms_active_fw_send(&ctx_gas,
            &input,
            &sdk_error,
            NULL,
            0);

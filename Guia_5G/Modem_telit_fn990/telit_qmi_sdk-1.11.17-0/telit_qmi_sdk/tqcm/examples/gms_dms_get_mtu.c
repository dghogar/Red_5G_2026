...
int main(int argc, char **argv)
{
    ...
    gms_dms_get_mtu_resp_t output = {0};
    uint16_t sdk_error;
    ...
    ret = tqcm_gms_dms_get_mtu_send(&ctx_gms,
            &output,
            &sdk_error,
            NULL,
            0);
    ...
    if (output.mtu_size_3gpp_available) {
        RLOGI("mtu size 3gpp: %u", output.mtu_size_3gpp);
    }
    if (output.mtu_size_hrpd_available) {
        RLOGI("mtu size hrpd: %u", output.mtu_size_hrpd);
    }

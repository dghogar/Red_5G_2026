...
int main(int argc, char **argv)
{
    ...
    input.latitude_available = true;
    input.latitude = 39.2179454;
    input.longitude_available = true;
    input.longitude = 9.0860885;
    input.hor_unc_circular_available = true;
    input.hor_unc_circular = 18.75;
    ...
    ret = tqcm_loc_inject_position_send(&ctx_loc, &input, &sdk_error, NULL, 0);
    if (ret != 0) {
        RLOGE("send fail %d", ret);
        if (ret == EPROTO) {
            RLOGE("qmi protocol error 0x%04X", sdk_error);

...
int main(int argc, char **argv)
{
    ...
    ret = tqcm_gms_nas_set_mccblock_send(&ctx_gms,
            &output,
            &sdk_error,
            NULL,
            0);
    ...
    RLOGI("mcc1: %u", output.mcc1);
    if (output.mcc2_available) {
        RLOGI("mcc2: %u", output.mcc2);
    }
    if (output.mcc3_available) {
        RLOGI("mcc3: %u", output.mcc3);
    }

...
int main(int argc, char **argv)
{
    ...
    uint16_t sdk_error;
    wds_profile_list_input_t input = { 0 };
    ...
    input.profile_type = QMI_WDS_PROFILE_TYPE_3GPP;
    input.profile_type_available = true;

    ret = tqcm_wds_get_profile_list_send(&ctx_wds, &input, &out, &sdk_error, NULL, 0);
    ...
    RLOGI("Found %u profiles", out.profile_list_size);

    for (i = 0; i < out.profile_list_size; i++) {
        RLOGI("Profile %u:", i + 1);
        RLOGI("    index: %u", out.profile_list[i].profile_index);
        RLOGI("    type: %s", out.profile_list[i].profile_type ? "3GPP2" : "3GPP");
        RLOGI("    description: %s", out.profile_list[i].profile_name);
    }

...
static uim_card_status_slot_app_t *find_ready_app_card_status(uim_card_status_info_t *out)
{
    uint8_t i, j, k;
    uim_card_status_slot_app_t *ret = NULL;
    ...
    if (out->card_status_available) {
        ...
        for (i = 0; i < out->card_status.num_slot; i++) {
            ...
            for (j = 0; j < out->card_status.slot[i].num_app; j++) {
                ...
                if (out->card_status.slot[i].app[j].state == QMI_UIM_APP_STATE_READY &&
                        out->card_status.slot[i].app[j].aid_len)
                    ret = &out->card_status.slot[i].app[j];
...
int main(int argc, char **argv)
{
    ...
    uint16_t sdk_error;
    uim_card_status_slot_app_t *app;
    ...
    app = find_ready_app_card_status(&out);
    if (app) {
        uint8_t aid_len;
        uint8_t aid[QMI_UIM_MAX_AID_LEN];

        aid_len = app->aid_len;
        memcpy(aid, app->aid, aid_len);

        RLOGI("##################");
        RLOGI("Disabling app");
        RLOGI("##################");
        uim_change_provisioning_session_t provisioning_input = {
            QMI_UIM_SESSION_TYPE_PRIMARY_GW_PROVISIONING,
            false
        };

        ret = tqcm_uim_change_provisioning_session_send(&ctx_uim,
                &provisioning_input,
                &sdk_error,
                NULL,
                0);
        ...
        RLOGI("Enabling app");
        RLOGI("##################");
        provisioning_input.application_info_available = true;
        provisioning_input.application_info.slot = 1;
        provisioning_input.application_info.aid_len = aid_len;
        memcpy(provisioning_input.application_info.aid, aid, aid_len);
        provisioning_input.activate = true;

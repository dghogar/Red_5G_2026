...
int main(int argc, char **argv)
{
    ...
    input.reason = QMI_VOICE_REASON_BARR_ALLOUTGOING;

    ret = tqcm_voice_get_call_barring_send(&ctx_voice,
            &input,
            &out,
            &sdk_error,
            NULL,
    ...
    for (i = 0; i < out.call_barring_service_size; i++) {
        RLOGI("service_class_ext[%u] %d", i, out.call_barring_service[i].service_class_ext);

        for (j = 0; j < out.call_barring_service[i].call_barring_number_list_size; j++) {
            RLOGI("active_status[%u][%u] %d", i, j,
                    out.call_barring_service[i].call_barring_number_list[j].active_status);
            RLOGI("number[%u][%u] %s", i, j, out.call_barring_service[i].call_barring_number_list[j].number);
        }
    }

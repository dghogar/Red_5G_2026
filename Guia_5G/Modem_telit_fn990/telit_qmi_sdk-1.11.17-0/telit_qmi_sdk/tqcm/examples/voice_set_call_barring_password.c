...
#define OLD_PASSWORD "0000"
#define NEW_PASSWORD "9999"

int main(int argc, char **argv)
{
    int ret = EXIT_SUCCESS;
    tqcm_client_context_t ctx_voice;
    voice_call_barring_input_t input = { 0 };
    ...
    input.reason = QMI_VOICE_REASON_BARR_ALLOUTGOINGBARRING;
    memcpy(input.old_password, OLD_PASSWORD, CALL_BARRING_PASSWORD_LEN);
    memcpy(input.new_password, NEW_PASSWORD, CALL_BARRING_PASSWORD_LEN);

    ret = tqcm_voice_set_call_barring_password_send(&ctx_voice,
            &input,
            &out,
            &sdk_error,
            NULL,

...
int main(int argc, char **argv)
{
    ...
    if (argc < 3) {
        RLOGE("Old and new PIN should be provided");
        ret = EXIT_FAILURE;
        goto end;
    } else if (argc == 3) {
    ...
    info.pin_info.old_pin_len = strlen(old);
    strlcpy(info.pin_info.old_pin, old, sizeof(info.pin_info.old_pin));
    info.pin_info.new_pin_len = strlen(new);
    strlcpy(info.pin_info.new_pin, new, sizeof(info.pin_info.new_pin));
    ...
    ret = tqcm_uim_change_pin_send(&ctx_uim, &info, &out, &sdk_error, NULL, 0);

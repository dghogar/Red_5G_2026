...
int main(int argc, char **argv)
{
    ...
    gms_loc_get_auto_start_resp_t output = {0};
    uint16_t sdk_error;
    ...
    ret = tqcm_gms_loc_get_auto_start_send(&ctx_gms,
            &output,
            &sdk_error,
            NULL,
            0);
    ...
    if (output.function_reported) {
        RLOGI("function: %u", output.function);
    }
    if (output.fix_type_reported) {
        RLOGI("fix_type: %u", output.fix_type);
    }

...
int main(int argc, char **argv)
{
    ...
    ret = tqcm_gas_dms_get_usb_cfg_send(&ctx_gas,
            &info,
            &sdk_error,
            NULL,
            0);
    if (ret != 0) {
        ...
        RLOGI("pid 0x%x", info.pid);
        RLOGI("hsic %u", info.hsic);
        RLOGI("persistence %u", info.persistence);
        RLOGI("immediate %u", info.immediate);
        RLOGI("reboot %u", info.reboot);
    }

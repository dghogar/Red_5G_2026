...
/* Make sure to have the service opened with CTL requests:
 * client id should be passed as argument
 */
    ...
    switch (rsp_ctx->type) {
        ...
        case QMI_MSG_RESPONSE:
            ...
            if (rsp_ctx->xid == xid) {
                ...
                rtn = telit_dms_get_device_rev_id_unpack(payload, payload_len, &out);
...
int main(int argc, char **argv)
{
    ...
    rtn = telit_dms_get_device_rev_id_pack(&req_ctx, req, &req_len);
    if (rtn != QMI_SDK_ERR_NONE) {
    ...
    while (!exit_wait) {
        ...
        if (p_fd.revents & POLLIN) {
            ...
            if (rtn > 0) {
                ...
                rtn = telit_helper_qmux_get_svc_cli_id(rsp, rsp_len, &svc, &cid);

                if (rtn != QMI_SDK_ERR_NONE) {
                    RLOGE("failed to get service and client id");
                    continue;
                }

                RLOGI("Service id: %d, client id: %d", svc, cid);

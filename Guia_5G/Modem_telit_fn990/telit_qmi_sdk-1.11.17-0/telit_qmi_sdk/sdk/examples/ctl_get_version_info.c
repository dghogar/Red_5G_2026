    ...
    switch (rsp_ctx->type) {
        ...
        case QMI_MSG_RESPONSE:
            ...
            if (rsp_ctx->xid == xid) {
                ...
                rtn = telit_ctl_get_version_info_unpack(payload, payload_len, &out);
                if (rtn != QMI_SDK_ERR_NONE) {
                    ...
                    for (i = 0; i < out.service_version_list_size; i++) {
                        RLOGI("qmi_svc_type: %u, major_ver: %u, minor_ver: %u",
...
int main(int argc, char **argv)
{
    ...
    rtn = telit_ctl_get_version_info_pack(&req_ctx, req, &req_len);
    if (rtn != QMI_SDK_ERR_NONE) {

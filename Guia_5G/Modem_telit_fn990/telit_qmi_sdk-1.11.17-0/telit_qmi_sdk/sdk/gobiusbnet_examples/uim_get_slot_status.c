...
int main()
{
    ...
    rtn = telit_uim_get_slots_status_pack(&req_ctx, req, &req_size);
        ...
        if ((rtn > 0) && (rsp_size > 0)) {
            ...
            switch (rsp_ctx.type) {
                case QMI_MSG_RESPONSE:
                    ...
                    if (rsp_ctx.xid == xid) {
                        rtn = telit_uim_get_slots_status_unpack(rsp, rsp_size, &out);
                        ...
                        if (rtn != QMI_SDK_ERR_NONE) {
                            ...
                            for (uint16_t i = 0; i < out.slot_size; i++) {
                                ...
                                telit_print_buffer(out.slot[i].iccid, out.slot[i].iccid_len, LOG_INFO);
                            }
                        }

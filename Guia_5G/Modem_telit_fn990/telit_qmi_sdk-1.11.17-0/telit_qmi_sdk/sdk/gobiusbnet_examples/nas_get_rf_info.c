...
int main()
{
    ...
    rtn = telit_nas_get_rf_band_info_pack(&req_ctx, req, &req_size);
        ...
        if ((rtn > 0) && (rsp_size > 0)) {
            ...
            switch (rsp_ctx.type) {
                case QMI_MSG_RESPONSE:
                    ...
                    if (rsp_ctx.xid == xid) {
                        rtn = telit_nas_get_rf_band_info_unpack(rsp, rsp_size, &out);
                        ...
                        if (rtn != QMI_SDK_ERR_NONE) {
                            ...
                            for (i = 0; i < out.rf_info_instances_size; i++) {
                                RLOGI("NAS radio_if %u", out.rf_info_instance[i].radio_if);
                                RLOGI("NAS active_band %u", out.rf_info_instance[i].active_band);
                                RLOGI("NAS active_channel %u", out.rf_info_instance[i].active_channel);
                            }

                            for (i = 0; i < out.rf_info_extended_size; i++) {
                                RLOGI("NAS extended radio_if %u", out.rf_info_extended[i].radio_if);
                                RLOGI("NAS extended active_band %u", out.rf_info_extended[i].active_band);
                                RLOGI("NAS extended active_channel %u",
                                        out.rf_info_extended[i].active_channel);
                            }
                        }

...
int main()
{
    ...
    rtn = telit_nas_get_plmn_name_pack(&req_ctx, req, &req_size, &input);
        ...
        if ((rtn > 0) && (rsp_size > 0)) {
            ...
            switch (rsp_ctx.type) {
                case QMI_MSG_RESPONSE:
                    ...
                    if (rsp_ctx.xid == xid) {
                        rtn = telit_nas_get_plmn_name_unpack(rsp, rsp_size, &out);
                        ...
                        if (rtn != QMI_SDK_ERR_NONE) {
                            ...
                            if (out.eons_plmn_name_available) {
                                ...
                                RLOGI("short_name_encoding %d",
                                        out.eons_plmn_name.name_short.short_name_encoding);
                                RLOGI("short_name_country_initials %d",
                                        out.eons_plmn_name.name_short.short_name_country_initials);
                                RLOGI("short_name_spare_bits %d",
                                        out.eons_plmn_name.name_short.short_name_spare_bits);
                                if (out.eons_plmn_name.name_short.short_name_len > 0) {
                                    RLOGI("short_name %s", out.eons_plmn_name.name_short.short_name);
                                }

                                RLOGI("long_name_encoding %d",
                                        out.eons_plmn_name.name_long.long_name_encoding);
                                RLOGI("long_name_country_initials %d",
                                        out.eons_plmn_name.name_long.long_name_country_initials);
                                RLOGI("long_name_long_bits %d",
                                        out.eons_plmn_name.name_long.long_name_long_bits);

...
/* ATTENTION: modify the following according to the sim card */
#define PUK "75087245"
#define PIN "1958"
...
int main()
{
    ...
    input.pin_info.puk_len = strlen(PUK);
    strlcpy(input.pin_info.puk, PUK, sizeof(input.pin_info.puk));
    input.pin_info.new_pin_len = strlen(PIN);
    strlcpy(input.pin_info.new_pin, PIN, sizeof(input.pin_info.new_pin));
    ...
    rtn = telit_uim_unblock_pin_pack(&req_ctx, req, &req_size, &input);
        ...
        if ((rtn > 0) && (rsp_size > 0)) {
            ...
            switch (rsp_ctx.type) {
                case QMI_MSG_RESPONSE:
                    ...
                    if (rsp_ctx.xid == xid) {
                        rtn = telit_uim_unblock_pin_unpack(rsp, rsp_size, &out);

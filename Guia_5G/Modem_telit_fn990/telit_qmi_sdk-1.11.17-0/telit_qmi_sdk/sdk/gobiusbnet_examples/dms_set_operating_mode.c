...
int main()
{
    ...
    input.power_mode = QMI_DMS_OP_MODE_ONLINE;
    ...
    rtn = telit_dms_set_operating_mode_pack(&req_ctx, req, &req_size, &input);
        ...
        if ((rtn > 0) && (rsp_size > 0)) {
            ...
            switch (rsp_ctx.type) {
                case QMI_MSG_RESPONSE:
                    ...
                    if (rsp_ctx.xid == xid) {
                        rtn = telit_dms_set_operating_mode_unpack(rsp, rsp_size);

                        if (rtn != QMI_SDK_ERR_NONE) {
                            RLOGE("%s unpack pack fail %d ", SVC_STR, rtn);
                        } else {
                            RLOGI("DMS set operating mode unpack ok");
                        }

...
int main()
{
    ...
    input.emergency_mode_available = true;
    input.emergency_mode_on = false;
    input.mode_pref_available = true;
    input.mode_pref = 15;

    telit_set_log_level(LOG_INFO);

    rtn = telit_nas_set_system_selection_preference_pack(&req_ctx, req, &req_size, &input);
        ...
        if ((rtn > 0) && (rsp_size > 0)) {
            ...
            switch (rsp_ctx.type) {
                case QMI_MSG_RESPONSE:
                    ...
                    if (rsp_ctx.xid == xid) {
                        rtn = telit_nas_set_system_selection_preference_unpack(rsp, rsp_size);

                        if (rtn != QMI_SDK_ERR_NONE) {
                            RLOGE("%s unpack pack fail %d ", SVC_STR, rtn);
                        } else {
                            RLOGI("NAS set system selection preference unpack ok");
                        }

...
int main(int argc __unused, char **argv __unused)
{
    ...
    rtn = telit_wms_list_messages_pack(&req_ctx, req, &req_size, &input);
        ...
        if ((rtn > 0) && (rsp_size > 0)) {
            ...
            switch (rsp_ctx.type) {
                case QMI_MSG_RESPONSE:
                    ...
                    if (rsp_ctx.xid == xid) {
                        rtn = telit_wms_list_messages_unpack(rsp, rsp_size, &out);
                        ...
                        if (rtn != QMI_SDK_ERR_NONE) {
                            ...
                            RLOGI("got %d messages", out.message_list_size);

                            for (uint32_t i = 0; i < out.message_list_size; i++) {
                                RLOGI("%d) message index %d", i, out.message_list[i].message_index);
                                RLOGI("tag type %d", out.message_list[i].tag_type);
                            }
                        }

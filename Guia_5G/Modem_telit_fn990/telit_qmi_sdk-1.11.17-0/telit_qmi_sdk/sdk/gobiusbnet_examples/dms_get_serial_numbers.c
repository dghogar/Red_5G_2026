...
int main()
{
    ...
    rtn = telit_dms_get_device_serial_numbers_pack(&req_ctx, req, &req_size);
        ...
        if ((rtn > 0) && (rsp_size > 0)) {
            ...
            switch (rsp_ctx.type) {
                case QMI_MSG_RESPONSE:
                    ...
                    if (rsp_ctx.xid == xid) {
                        rtn = telit_dms_get_device_serial_numbers_unpack(rsp, rsp_size, &out);

                        if (rtn != QMI_SDK_ERR_NONE) {
                            RLOGE("%s unpack pack fail %d ", SVC_STR, rtn);
                        } else {
                            RLOGI("DMS esn %s, %u", out.esn, out.esn_len);
                            RLOGI("DMS imei %s, %u", out.imei, out.imei_len);
                            RLOGI("DMS meid %s, %u", out.meid, out.meid_len);
                            RLOGI("DMS imei svn %s, %u", out.imei_svn, out.imei_svn_len);
                        }

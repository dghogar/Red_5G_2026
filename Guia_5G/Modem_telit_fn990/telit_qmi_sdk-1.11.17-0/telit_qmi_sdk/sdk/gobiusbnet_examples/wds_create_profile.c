...
int main()
{
    ...
    rtn = telit_wds_create_profile_pack(&req_ctx, req, &req_size, &input);
        ...
        if ((rtn > 0) && (rsp_size > 0)) {
            ...
            switch (rsp_ctx.type) {
                case QMI_MSG_RESPONSE:
                    ...
                    if (rsp_ctx.xid == xid) {
                        rtn = telit_wds_create_profile_unpack(rsp, rsp_size, &out);

                        if (rtn != QMI_SDK_ERR_NONE) {
                            RLOGE("%s unpack fail %d ", SVC_STR, rtn);

                            if (out.extended_error_code_available) {
                                RLOGI("Extended error code %u", out.extended_error_code);
                            }
                        } else {
                            RLOGI("%s unpack ok", SVC_STR);

                            if (out.profile_id_available) {
                                RLOGI("Profile type %s", out.profile_id.type ? "3GPP2" : "3GPP");

...
int main()
{
    ...
    input.pin_info.old_pin_len = strlen(OLD_PIN);
    strlcpy(input.pin_info.old_pin, OLD_PIN, sizeof(input.pin_info.old_pin));
    input.pin_info.new_pin_len = strlen(NEW_PIN);
    strlcpy(input.pin_info.new_pin, NEW_PIN, sizeof(input.pin_info.new_pin));

    telit_set_log_level(LOG_INFO);

    rtn = telit_uim_change_pin_pack(&req_ctx, req, &req_size, &input);
        ...
        if ((rtn > 0) && (rsp_size > 0)) {
            ...
            switch (rsp_ctx.type) {
                case QMI_MSG_RESPONSE:
                    ...
                    if (rsp_ctx.xid == xid) {
                        rtn = telit_uim_change_pin_unpack(rsp, rsp_size, &out);

                        if (rtn != QMI_SDK_ERR_NONE) {
                            RLOGE("%s unpack fail %d ", SVC_STR, rtn);

                            if (out.pin_retries_available) {
                                RLOGE("verify left %u, unblock left %u", out.retries_remaining.verify_left,
                            ...
                            if (out.encrypted_pin1_available) {
                                telit_print_buffer(out.encrypted_pin1.enc_pin1,
                                        out.encrypted_pin1.enc_pin1_len, LOG_INFO);
                            }
                        }

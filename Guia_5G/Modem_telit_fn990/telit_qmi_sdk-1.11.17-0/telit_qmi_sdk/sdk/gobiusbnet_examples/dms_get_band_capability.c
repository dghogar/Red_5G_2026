...
int main()
{
    ...
    rtn = telit_dms_get_band_capability_pack(&req_ctx, req, &req_size);
        ...
        if ((rtn > 0) && (rsp_size > 0)) {
            ...
            switch (rsp_ctx.type) {
                case QMI_MSG_RESPONSE:
                    ...
                    if (rsp_ctx.xid == xid) {
                        rtn = telit_dms_get_band_capability_unpack(rsp, rsp_size, &out);

                        if (rtn != QMI_SDK_ERR_NONE) {
                            RLOGE("%s unpack pack fail %d ", SVC_STR, rtn);
                        } else {
                            uint16_t i;
                            RLOGI("DMS out %lu (0x%08X)", out.band_capabilities, out.band_capabilities);
                            if (out.tds_band_capabilities_available) {
                                RLOGI("DMS TDS out %lu (0x%08X)", out.tds_band_capabilities,
                                        out.tds_band_capabilities);
                            }

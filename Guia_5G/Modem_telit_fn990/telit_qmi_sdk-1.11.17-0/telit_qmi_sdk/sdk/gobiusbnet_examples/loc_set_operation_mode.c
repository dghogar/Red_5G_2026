...
int main()
{
    ...
    input.operation_mode = QMI_LOC_OPER_MODE_DEFAULT;

    telit_set_log_level(LOG_INFO);

    rtn = telit_loc_set_operation_mode_pack(&req_ctx, req, &req_size, &input);
        ...
        if ((rtn > 0) && (rsp_size > 0)) {
            ...
            switch (rsp_ctx.type) {
                case QMI_MSG_RESPONSE:
                    ...
                    if (rsp_ctx.xid == xid) {
                        rtn = telit_loc_set_operation_mode_unpack(rsp, rsp_size);

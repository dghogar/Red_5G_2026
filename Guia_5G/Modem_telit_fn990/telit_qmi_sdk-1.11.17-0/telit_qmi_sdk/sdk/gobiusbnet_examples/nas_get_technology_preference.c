...
int main()
{
    ...
    rtn = telit_nas_get_technology_preference_pack(&req_ctx, req, &req_size);
        ...
        if ((rtn > 0) && (rsp_size > 0)) {
            ...
            switch (rsp_ctx.type) {
                case QMI_MSG_RESPONSE:
                    ...
                    if (rsp_ctx.xid == xid) {
                        rtn = telit_nas_get_technology_preference_unpack(rsp, rsp_size, &out);

                        if (rtn != QMI_SDK_ERR_NONE) {
                            RLOGE("%s unpack pack fail %d ", SVC_STR, rtn);
                        } else {
                            RLOGI("NAS tech_pref 0x%x", out.technology_preference);
                            RLOGI("NAS duration %u", out.technology_preference_duration);
                            if (out.persistent_available) {
                                RLOGI("NAS persistent_tech_pref %u", out.persistent);
                            }
                        }

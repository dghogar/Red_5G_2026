...
int main()
{
    ...
    input.ip_preference = QMI_IP_FAMILY_V4;

    telit_set_log_level(LOG_INFO);

    rtn = telit_wds_set_client_ip_family_pref_pack(&req_ctx, req, &req_size, &input);
        ...
        if ((rtn > 0) && (rsp_size > 0)) {
            ...
            switch (rsp_ctx.type) {
                case QMI_MSG_RESPONSE:
                    ...
                    if (rsp_ctx.xid == xid) {
                        rtn = telit_wds_set_client_ip_family_pref_unpack(rsp, rsp_size);

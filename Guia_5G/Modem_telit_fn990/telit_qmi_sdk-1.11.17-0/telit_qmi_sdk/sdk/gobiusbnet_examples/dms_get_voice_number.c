...
int main()
{
    ...
    rtn = telit_dms_get_msisdn_pack(&req_ctx, req, &req_size);
        ...
        if ((rtn > 0) && (rsp_size > 0)) {
            ...
            switch (rsp_ctx.type) {
                case QMI_MSG_RESPONSE:
                    ...
                    if (rsp_ctx.xid == xid) {
                        rtn = telit_dms_get_msisdn_unpack(rsp, rsp_size, &out);

                        if (rtn != QMI_SDK_ERR_NONE) {
                            RLOGE("%s unpack pack fail %d ", SVC_STR, rtn);
                        } else {
                            RLOGI("DMS voice number %s, %u", out.voice_number, out.voice_number_len);
                            RLOGI("DMS mobile id %s, %u", out.mobile_id, out.mobile_id_len);
                        }

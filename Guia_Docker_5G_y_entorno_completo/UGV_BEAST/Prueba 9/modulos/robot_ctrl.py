...
def procesador_hardware():
    while True:
        ...
        try:
            ...
            log.error(f"[ROBOT_CTRL] Error en bus de hardware: {e}")
        finally:
            cola_hw.task_done()
...
def vincular_navegador(nav):
    """Guarda la referencia del navegador para poder mandarle órdenes desde la radio"""
    global nav_thread_ref
    nav_thread_ref = nav

def parada_tactica_global():
    """Detiene la misión autónoma y clava los frenos del hardware"""
    if nav_thread_ref: 
        nav_thread_ref.stop_nav()
    mover_chasis("STOP")
...
def obtener_telemetria():
    ...
    return {
        ...
        "nav_state": estado_global.get("nav_state", "ESPERANDO"), 
        "telemetry": {

...
import modulos.robot_ctrl as rc  # IMPORTAMOS TU BUS SEGURO
...
class ModuloNavegacion(threading.Thread):
    ...
    def stop_nav(self):
        ...
        # EXTIRPADO: Ya no escribimos directo en self.base para evitar el crasheo
        log.info("[NAV] Misión detenida desde IA.")
    ...
    def run(self):
        ...
        while True:
            ...
            if self.state.get('wifi_critico', False):
                log.warning("[NAV] SEÑAL WIFI DÉBIL. Abortando.")
                self.stop_nav()
                rc.mover_chasis("STOP") # Parada segura
                continue 

            if self.state.get('obstaculo_critico', False):
                log.warning("[NAV] Evasión LiDAR: Muro detectado.")
                rc.mover_chasis_dinamico(-0.4, -0.4)
                time.sleep(1.0)
                rc.mover_chasis_dinamico(0.4, -0.4)
                time.sleep(1.0)
                rc.mover_chasis_dinamico(0, 0)
                continue
            ...
            # ENVIAMOS POR EL BUS SEGURO
            rc.mover_chasis_dinamico(round(L_f, 3), round(R_f, 3))
            time.sleep(0.05)